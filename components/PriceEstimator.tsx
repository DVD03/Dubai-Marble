    ...
    <div
      className={`rounded-3xl p-6 sm:p-10 border shadow-2xl transition-all ${
        isDark
          ...
          : "bg-[#F4F7FF] border-[#E5E7EB] text-[#1F2937]"
      }`}
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-100 dark:border-white/10">
        <div>
          <div className="inline-flex items-center gap-2 text-[#123B6D] text-xs font-bold uppercase tracking-widest mb-1.5">
            <Layers className="w-4 h-4 text-[#123B6D]" />
            <span>Inspection Scope Selector</span>
            <span className="font-arabic font-normal text-[11px] text-[#6B7280]">تحديد نطاق المعاينة</span>
          </div>
          <h3 className={`text-2xl sm:text-3xl font-black ${isDark ? "text-white" : "text-[#1F2937]"}`}>
            Customize Your Stone Restoration
          </h3>
        </div>
        <div className="bg-[#F4F7FF] border border-[#123B6D]/30 text-[#123B6D] px-4 py-1.5 rounded-full text-xs font-bold shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-7 space-y-6">
          {/* Step 1: Stone Type */}
          <div>
            <label className={`block text-xs font-bold uppercase tracking-wider mb-2 ${isDark ? "text-gray-300" : "text-[#6B7280]"}`}>
              1. Select Stone Type <span className="font-arabic lowercase text-[#6B7280] font-normal">نوع الرخام / الحجر</span>
            </label>
              ...
              {stones.map((s) => (
                <button
                  ...
                  className={`text-left px-4 py-3 rounded-xl text-xs font-semibold border transition-all ${
                    selectedStone === s.id
                      ? "bg-[#123B6D] text-white font-bold border-transparent shadow-md scale-[1.02]"
                      : isDark
                      ? "bg-white/5 border-white/10 text-gray-300 hover:border-[#123B6D]"
                      : "bg-[#F4F7FF] border-[#E5E7EB] text-[#1F2937] hover:border-[#123B6D]"
                  }`}
                >
                  {s.name}
                </button>
          ...
          <div>
            <label className={`block text-xs font-bold uppercase tracking-wider mb-2 ${isDark ? "text-gray-300" : "text-[#6B7280]"}`}>
              2. Select Restoration Service <span className="font-arabic lowercase text-[#6B7280] font-normal">نوع الخدمة</span>
            </label>
            <div className="space-y-2">
              {services.map((svc) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl text-xs font-semibold border flex items-center justify-between transition-all ${
                    selectedService === svc.id
                      ? "bg-[#123B6D] text-white font-bold border-transparent shadow-md"
                      : isDark
                      ? "bg-white/5 border-white/10 text-gray-300 hover:border-[#123B6D]"
                      : "bg-[#F4F7FF] border-[#E5E7EB] text-[#1F2937] hover:border-[#123B6D]"
                  }`}
                >
                  <span>{svc.name}</span>
                  <Sparkles className="w-3.5 h-3.5 text-[#123B6D] shrink-0" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className={`text-xs font-bold uppercase tracking-wider ${isDark ? "text-gray-300" : "text-[#6B7280]"}`}>
                3. Approximate Area (Square Meters) <span className="font-arabic lowercase text-[#6B7280] font-normal">المساحة</span>
              </label>
              <span className={`font-extrabold text-lg ${isDark ? "text-[#123B6D]" : "text-[#1F2937]"}`}>{sqm} m² ({Math.round(sqm * 10.764)} sq.ft)</span>
            </div>
            <input
              type="range"
              min="10"
              ...
              className="w-full h-2.5 bg-gray-200 dark:bg-white/10 rounded-lg appearance-none cursor-pointer accent-[#123B6D]"
            />
        ...
        <div className="lg:col-span-5">
          <div className={`p-7 rounded-2xl border text-center relative overflow-hidden shadow-xl ${
            isDark ? "bg-[#0000CC]/20 border-white/15" : "bg-[#F4F7FF] border-[#E5E7EB]"
          }`}>
            <div className="absolute top-0 inset-x-0 h-1 bg-[#123B6D]" />
            <div className="text-xs uppercase tracking-widest font-bold text-[#6B7280] mb-2">
              Tailored Consultation · المعاينة المجانية
            </div>
            <div className={`text-2xl sm:text-3xl font-black my-2 tracking-tight ${isDark ? "text-white" : "text-[#1F2937]"}`}>
              {stoneObj.name.split("/")[0]}
            </div>
            <div className="text-xs text-[#123B6D] mb-6 font-semibold">
              {servObj.name} · {sqm} m²
            </div>

            <div className={`space-y-2.5 text-left text-xs mb-6 border-y py-4 ${isDark ? "text-gray-300 border-white/10" : "text-[#6B7280] border-[#E5E7EB]"}`}>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#123B6D] shrink-0" />
            ...
            <Link
              ...
              className="w-full bg-[#123B6D] hover:bg-[#0000CC] text-white font-bold text-sm py-3.5 rounded-lg shadow-sm flex items-center justify-center gap-2 transition-colors"
            >
