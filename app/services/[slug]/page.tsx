          ...
          <div className="lg:col-span-8">
            <AnimatedSection>
              ...
              <div className="relative">
                <div className="absolute left-5 top-4 bottom-4 w-0.5 bg-gradient-to-b from-gold to-transparent hidden sm:block" />
                <div className="flex flex-col gap-6">
                  {service.process.map((step, idx) => (
                    <div key={step.step} className="flex gap-5">
                      <div className="w-10 h-10 rounded-full gold-gradient flex items-center justify-center font-black text-dark text-sm shrink-0 shadow z-10">
                        {idx + 1}
                      </div>
              ...
              <div className="mt-10 p-6 rounded-2xl bg-[#f8f7f4] border-l-4 border-gold">
                <h4 className="font-bold text-dark text-base mb-1">Book a Free On-Site Inspection</h4>
                <p className="text-sm text-gray-500 mb-4">
                  Our Dubai technician will visit your property, measure the area, and provide a fixed-price quotation.
                </p>
                <Link href="/booking" className="gold-gradient text-dark font-bold text-xs px-6 py-3 rounded-xl shadow inline-flex items-center gap-2">
          ...
          <div className="lg:col-span-4 flex flex-col gap-7">
            <AnimatedSection direction="right">
              <div className="bg-[#f8f7f4] border border-gray-200 rounded-2xl p-6">
                <h3 className="font-black text-dark text-base mb-4 pb-3 border-b border-gray-200">All Services</h3>
                    ...
                    <Link key={s.slug} href={`/services/${s.slug}`}
                      className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-colors ${
                        s.slug === service.slug
                          ...
                          : "bg-white text-gray-600 hover:bg-gray-100 hover:text-gold"
                      }`}>
                      {s.title} <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <AnimatedSection direction="right" delay={0.1}>
              <div className="relative rounded-2xl overflow-hidden">
                ...
                <div className="absolute inset-0 bg-dark/85 flex flex-col justify-end p-6">
                  <h3 className="font-black text-white text-base mb-2">Need Immediate Advice?</h3>
                  <p className="text-xs text-gray-300 mb-4">Speak directly with our senior stone technician.</p>
                  <a href="tel:+971522774953"
                    className="gold-gradient text-dark font-bold text-xs py-3 rounded-xl shadow w-full flex items-center justify-center gap-2 mb-2">
